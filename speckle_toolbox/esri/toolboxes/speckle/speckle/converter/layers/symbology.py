...
def cadBimRendererToNative(
    project: ArcGISProject,
    active_map,
    layerGroup,
    fetColors: List[RenderMaterial],
    layerArcgis,
    f_class,
    existingAttrs: List,
        ...
        if layerArcgis.isFeatureLayer:
            ...
            for k, grp in enumerate(sym.renderer.groups):
                for itm in grp.items:
                    transVal = itm.values[0][
                        0
                    ]  # Grab the first "percent" value in the list of potential values
                    # print(transVal)
                    for i in range(len(class_shapes)):
                        label = class_shapes[i]
                        # print(label)
                        if label is None or label == "" or str(label) == "":
                            label = "<Null>"
...
def vectorRendererToNative(
    ...
    active_map,
    layerGroup,
    layerSpeckle: Union[Layer, VectorLayer],
    layerArcgis,
    f_class,
    existingAttrs: List,
) -> Union[None, Dict[str, Any]]:
            ...
            if layerArcgis.isFeatureLayer:
                ...
                if renderer["type"] == "singleSymbol":
                    ...
                    r, g, b = get_rgb_from_speckle(
                        renderer["properties"]["symbol"]["symbColor"]
                    )
                    # print(r,g,b)
                    ...
                    if gradMetod != 0:
                        ...
                        layerArcgis.symbology = sym  # SimpleRenderer
                        return layerArcgis
                    if attribute not in existingAttrs or gradMetod != 0:
                    ...
                    if len(sym.renderer.classBreaks) > 0:
                        ...
                        for k, br in enumerate(ranges):
                            ...
                            if sym.renderer.breakCount < len(ranges):
                                ...
                                for val in all_values:
                                    if val <= ranges[k]["upper"] and (
                                        totalClasses == 0
                                        or (
                                            totalClasses > 0
                                            and sym.renderer.classBreaks[
                                                totalClasses - 1
                                            ].upperBound
                                            < val
                                        )
                            ...
                            sym.renderer.classBreaks[totalClasses].upperBound = ranges[
                                k
                            ]["upper"]
                            sym.renderer.classBreaks[totalClasses].label = ranges[k][
                                "label"
                            ]
                            sym.renderer.classBreaks[totalClasses].symbol.color = {
                                "RGB": [r, g, b, 100]
                            }
                            totalClasses += 1
                        ...
                        sym.renderer.classBreaks[0].upperBound = ranges[0][
                            "upper"
                        ]  # otherwise its assigned maximum value
        ...
        if renderer and renderer["type"]:
            ...
            if renderer["type"] == "singlebandgray":
                ...
                if symJson is None:
                    ...
                    temp = arcpy.management.MakeRasterLayer(
                        rasterPathsToMerge[band_index], newName + "_temp"
                    ).getOutput(0)
                ...
                if symJson is None:
                    ...
                    redSt = copy.deepcopy(
                        symJson["layerDefinitions"][0]["colorizer"]["stretchStatsRed"]
                    )
                    greenSt = copy.deepcopy(
                        symJson["layerDefinitions"][0]["colorizer"]["stretchStatsGreen"]
                    )
                    blueSt = copy.deepcopy(
                        symJson["layerDefinitions"][0]["colorizer"]["stretchStatsBlue"]
                    )
                    ...
                    try:
                        symJson["layerDefinitions"][0]["colorizer"][
                            "greenBandIndex"
                        ] = (greenBand - 1)
                    except:
                        symJson["layerDefinitions"][0]["colorizer"][
                    ...
                    try:
                        symJson["layerDefinitions"][0]["colorizer"]["redBandIndex"] = (
                            redBand - 1
                        )
                    except:
                        symJson["layerDefinitions"][0]["colorizer"]["redBandIndex"] = 0

                    try:
                        symJson["layerDefinitions"][0]["colorizer"]["blueBandIndex"] = (
                            blueBand - 1
                        )
                    except:
                    ...
                    for br in sym.colorizer.groups:
                        ...
                        for k, itm in enumerate(br.items):
                            if k < len(renderer["properties"]["classes"]):
                                ...
                                for n, cl in enumerate(
                                    renderer["properties"]["classes"]
                                ):
                                    if k == n:
...
def rendererToSpeckle(project: ArcGISProject, active_map, arcLayer, rasterFeat: Base):
    ...
    try:
        if arcLayer.isRasterLayer:
            try:
                rType = (
                    arcLayer.symbology.colorizer.type
                )  # 'singleSymbol','categorizedSymbol','graduatedSymbol',
                if rType == "RasterStretchColorizer":
                    rType = "singlebandgray"
                elif rType == "RasterUniqueValueColorizer":
                    rType = "paletted"  # only for 1-band raster
                else:
                    rType = "singlebandgray"
            except:
            ...
            bandValues = [
                rasterFeat["@(10000)" + name + "_values"] for name in rasterBandNames
            ]

            if rType == "singlebandgray":
                try:
                    band = arcLayer.symbology.colorizer.band
                except:
                    band = 0
                try:
                ...
                try:
                    greenBand = (
                        symJson["layerDefinitions"][0]["colorizer"]["greenBandIndex"]
                        + 1
                    )
                except:
                    greenBand = None
                try:
                    blueBand = (
                        symJson["layerDefinitions"][0]["colorizer"]["blueBandIndex"] + 1
                    )
                except:
                    blueBand = None
                try:
                    redBand = (
                        symJson["layerDefinitions"][0]["colorizer"]["redBandIndex"] + 1
                    )
                except:
                    print(greenBand)
                    print(blueBand)
                    if blueBand != 1 and greenBand != 1:
                        redBand = 1
                    else:
                        redBand = None
                print(redBand)
                ...
                layerRenderer["properties"].update(
                    {"redContrast": 1, "redMin": rbvalMin, "redMax": rbvalMax}
                )
                layerRenderer["properties"].update(
                    {"greenContrast": 1, "greenMin": gbvalMin, "greenMax": gbvalMax}
                )
                layerRenderer["properties"].update(
                    {"blueContrast": 1, "blueMin": bbvalMin, "blueMax": bbvalMax}
                )
            elif rType == "paletted":
                band = 0
                    ...
                    if cl.heading == "Value":
                        for k, itm in enumerate(cl.items):
                            ...
                            try:
                                ...
                                sColor = (r << 16) + (g << 8) + b
                                classes.append(
                                    {"color": sColor, "value": value, "label": label}
                                )
                            except:
                                pass
                layerRenderer.update(
            ...
            if sym.renderer.type == "SimpleRenderer":
                ...
                sourceSymbColor = symbol_color_to_speckle(
                    sym.renderer.defaultSymbol.color
                )
                ...
                or sym.renderer.type == "GraduatedSymbolsRenderer"
            ):
                ...
                layerRenderer["properties"][
                    "ramp"
                ] = {}  # gradientColorRampToSpeckle(rRamp)
                ...
                for itm in rRanges:
                    try:
                        lower = (
                            float(itm.label.split(" - ")[0])
                            if (" - " in rRanges.label)
                            else float(rRanges.label[0])
                        )
                    except:
                        lower = 0
        ...
        if sym.renderer.type == "SimpleRenderer":
            ...
            for i, grp in enumerate(categories):
                ...
                for n, itm in enumerate(grp.items):
                    for k, attrs in enumerate(row_shapes_list):
                        if str(itm.values[0][0]) == "<Null>":
                            itm.values[0][0] = None
                        if k == index and (
                            str(attrs[0]) == str(itm.values[0][0])
                            or (attrs[0] is None and str(itm.values[0][0]) == "<Null>")
                        ):
            ...
            for itm in rRanges:
                ...
                for k, attrs in enumerate(row_shapes_list):
                    try:
                        if (
                            k == index
                            and float(attrs[0]) <= float(itm.upperBound)
                            and (k == 0 or float(attrs[0]) > float(upperBounds[-1]))
                        ):
                            color = itm.symbol.color
                            color_found = 1
                            break
            ...
            for itm in rRanges:
                ...
                try:
                    if (
                        row_range != 0
                        and float(feat_value) <= float(itm.upperBound)
                        and (
                            len(upperBounds) == 0
                            or float(feat_value) > float(upperBounds[-1])
                        )
                    ):
                        rgb = 255 - int((itm.upperBound - row_min) / row_range * 255)
