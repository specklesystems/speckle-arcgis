...
from speckle.speckle.converter.layers.feature import (
    featureToNative,
    featureToSpeckle,
    cadFeatureToNative,
    bimFeatureToNative,
...
def convertSelectedLayers(
    layers: List[arcLayer], project: ArcGISProject
) -> List[Union[VectorLayer, Layer]]:
...
def layerToSpeckle(
    layer: arcLayer, project: ArcGISProject
) -> Union[
    ...
    try:
        ...
        speckleReprojectedCrs = CRS(
            name=projectCRS.name, wkt=projectCRS.exportToString(), units=units
        )
        ...
        if layer.isFeatureLayer:
            ...
            speckleLayer.renderer = rendererToSpeckle(
                project, project.activeMap, layer, None
            )
            ...
            print(
                arcpy.Describe(layer.dataSource)
            ...
            speckleLayer.renderer = rendererToSpeckle(
                project, project.activeMap, layer, b
            )
...
def layerToNative(layer: Any, streamBranch: str, plugin=None) -> arcLayer:
    ...
    try:
        ...
        try:
            ...
            if collectionType is None or collectionType == "None":
                raise Exception
        except:
            # print("except")
            ...
            for f in layer_elements:
                ...
                if isinstance(f["geometry"], Base):
                    try:
                        ...
                        bound = f[
                            "geometry"
                        ].boundary  # polygon found, default to receiving VectorLayer
                        print(bound)
                        if bound is None:
                            raise Exception
                        break
                    ...
                    for v in f["geometry"]:
                        try:
                            ...
                            bound = (
                                v.boundary
                            )  # polygon found, default to receiving VectorLayer
                            print(bound)
                            if bound is None:
                                raise Exception
                            break
                                            ...
                                            for el in layer_elements:
                                                for el_g in el.geometry:
                                                    for el_g.d in el_g.displayValue:
                                                        if isinstance(el_g.d, Mesh):
                                                            meshes.append(el_g.d)
                                            bimLayerToNative(
                                                meshes,
                                                layer.name,
                                                streamBranch,
...
def bimLayerToNative(
    ...
    try:
        ...
        if len(geom_meshes) > 0:
            layer_meshes = bimVectorLayerToNative(
                geom_meshes, layerName, "Mesh", streamBranch, project, sr
            )
    ...
    try:
        ...
        if sr is None:
            sr = arcpy.SpatialReference(
                text=active_map.spatialReference.exportToString()
            )
        ...
        # findOrCreatePath(p)
        path_bim = (
            p + "\\Layers_Speckle\\BIM_layers\\" + streamBranch + "\\" + newName + "\\"
        )  # arcpy.env.workspace + "\\" #
        ...
        # shp = meshToNative(geomList, path_bim + newName, dataStorage)
        ...
        for f in geomList[:]:
            try:
                ...
                if exist_feat is None:
                    arcpy.AddError(
                        f'Feature "{shape_id}" skipped due to invalid geometry'
                    )
                    print(shape_id)
...
def cadLayerToNative(
    layerContentList: List[Base], layerName: str, streamBranch: str, plugin=None
):
    ...
    try:
        ...
        if len(geom_points) > 0:
            layer_points = cadVectorLayerToNative(
                geom_points, layerName, "Points", streamBranch, project
            )
        if len(geom_polylines) > 0:
            layer_polylines = cadVectorLayerToNative(
                geom_polylines, layerName, "Polylines", streamBranch, project
            )
    except Exception as e:
        logToUser(str(e), level=2, func=inspect.stack()[0][3])
...
def cadVectorLayerToNative(
    geomList, layerName: str, geomType: str, streamBranch: str, project: ArcGISProject
):
    ...
    try:
        ...
        if "polygon" in geomType.lower():
            geomType = "Polygon"
        elif "line" in geomType.lower():
            geomType = "Polyline"
        elif "multipoint" in geomType.lower():
            geomType = "Multipoint"
